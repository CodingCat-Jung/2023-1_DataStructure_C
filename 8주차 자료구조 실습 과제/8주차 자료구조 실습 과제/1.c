/* 
자료구조 8주차 문제 1번 - 20204026 정명훈
다음과 같이 단순 연결 리스트에 사용자가 입력하는 값을 저장 했다가 출력하는 프로그램을 작성하라
*/
// 연결리스트 코드 복사 시작지점
#include <stdio.h>
#include <stdlib.h>
...
void print_list(ListNode* head) {
	ListNode* p = head; // p를 head 가리키도록 
	printf("생성된 연결 리스트 : ");

	while (p != NULL) { // p가 NULL일 때 까지
		printf("%d ", p->data); // p가 가리키는 데이터 출력
		p = p->link; // 그 다음 노드로 이동
		printf("-> ");
	}
	printf("\n");
}
